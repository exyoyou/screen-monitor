...
class StorageRepositoryImpl(
    ...
    private suspend fun migrateStorage(
        ...
        val otherDirs =
            oldBaseDir.listFiles()?.filter { it.isDirectory && it.name !in knownOldNames }
                ?: emptyList()
            ...
            try {
                if (file.isDirectory) {
                    ...
                    val targetName = if (preferExternal) {
                        if (file.name.contains("tmp_")) file.name else {
                            ...
                            if (lastDotIndex > 0) {
                                file.name.substring(
                                    0,
                                    lastDotIndex
                                ) + ".tmp_" + file.name.substring(lastDotIndex + 1)
                            } else {
                                file.name
                            }
                        }
                        ...
                        if (file.name.contains("tmp_")) {
                            val lastDotIndex = file.name.lastIndexOf('.')
                            if (lastDotIndex > 0 && file.name.substring(lastDotIndex)
                                    .startsWith(".tmp_")
                            ) {
                                file.name.substring(0, lastDotIndex) + "." + file.name.substring(
                                    lastDotIndex + 5
                                )
                            } else {
                                file.name
                ...
                val targetName = if (preferExternal) {
                    if (file.name.contains("tmp_")) file.name else {
                        val lastDotIndex = file.name.lastIndexOf('.')
                        if (lastDotIndex > 0) {
                            file.name.substring(0, lastDotIndex) + ".tmp_" + file.name.substring(
                                lastDotIndex + 1
                            )
                        } else {
                            file.name
